...
function reorderItems(newIndex) {
    ...
    // Capture positions of all items BEFORE the DOM change (FLIP: First)
    var firstPositions = [];
    for (var i = 0; i < dragState.items.length; i++) {
        if (dragState.items[i] !== dragState.element) {
            firstPositions.push({
                el: dragState.items[i],
                top: dragState.items[i].getBoundingClientRect().top
            });
        }
    }
    ...
    // FLIP: Last + Invert + Play — animate items to their new positions
    for (var j = 0; j < firstPositions.length; j++) {
        var item = firstPositions[j];
        var lastTop = item.el.getBoundingClientRect().top;
        var deltaY = item.top - lastTop;
        if (deltaY !== 0) {
